...
def train_production_model(df: pd.DataFrame) -> CatBoostClassifier:
    ...
    print(f"Starting Optuna hyperparameter optimization ({settings.optuna_n_trials} trials)...")
    ...
    with mlflow.start_run(run_name="CatBoost_Production"):
        ...
        mlflow.catboost.log_model(final_model, artifact_path="model", signature=signature)
