...
def build_faiss_index(df: pd.DataFrame):
    ...
    texts = df.apply(
        lambda row: f"{row['name']} is a {row['industry']} startup located in {row['country']} with {row['co_investor_count']} co-investors.",
        axis=1,
    ).tolist()
    ...
    try:
        ...
        print(
            f"Skipping FAISS indexing due to SentenceTransformer/PyTorch OS Error: {e}"
        )
        return
...
def main():
    ...
    train_production_model(df_clean)
