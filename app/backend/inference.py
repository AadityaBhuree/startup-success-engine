...
class InferenceEngine:
    ...
    def load_model(self):
        ...
        if mlflow_uri == "fallback":
            logger.warning("MLflow is disabled, skipping model loading.")
            return
    ...
    def load_faiss(self):
        try:
            ...
            if os.path.exists(index_path) and os.path.exists(meta_path):
                ...
                logger.warning(
                    "FAISS index not found. Recommendations will use fallback."
                )
        except Exception as e:
                ...
                for idx, dist in zip(indices[0], distances[0]):
                    if idx < len(self.faiss_metadata):
                        ...
                        recommendations.append(
                            {"name": meta["name"], "similarity": round(similarity, 2)}
                        )
